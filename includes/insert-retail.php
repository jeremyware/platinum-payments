...
if (!empty($dbaUserName) || !empty($companyStreet) || !empty($companyStreetLine2) || !empty($companyCity) || !empty($companyState) || !empty($companyZip)) {
 ...
 if (mysqli_connect_error()) {
  ...
  // $SELECT = "SELECT * From PlatinumPaymentsRetailInfo Where dbaUserName = ? Limit 1";
  // $INSERT = "INSERT Into PlatinumPaymentsRetailInfo (dbaUserName, companyStreet, companyStreetLine2, companyState, companyCity, companyZip) values(?, ?, ?, ?, ?, ?)";

  $SELECT = "SELECT dbaUserName From PlatinumPaymentsRetailInfo Where dbaUserName = ? Limit 1";
  $INSERT = "INSERT Into PlatinumPaymentsRetailInfo (dbaUserName, companyStreet, companyStreetLine2, companyState, companyCity, companyZip) values(?, ?, ?, ?, ?, ?)";
